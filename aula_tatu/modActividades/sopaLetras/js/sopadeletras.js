...
function inicializar(palabras) {
    var N = palabras.length; //verificamos el tamao de la palabra mas grande
    var max = 0;
    for (var i = 0; i<N; i++) if (palabras[i].length>max) max = palabras[i].length; //los caracteres especiales cuentan como uno
    window.tam =  max + ((N<10)?N : N-5); //tamanio de la matricula 
    //window.tam = max+2;
    sopa = new Array(window.tam);//inicializamos la cuadricula de la sopa
    for (var i = 0; i< window.tam; i++) window.sopa[i] = new Array(window.tam);
    for (var i = 0; i<window.tam; i++) for (var j=0; j<window.tam; j++) window.sopa[i][j] = "0";
}
...
enc = false;
    while (!enc) {
        ...
        inicio = 0;
        if (palabra.sentido === 1) {palabra.fila = pos2; palabra.columna = pos1; }
        else{ palabra.fila = pos1; palabra.columna = pos2; }
        if (libre(palabra)) { colocarPalabra(palabra); return palabra;}
    }
    return palabra;
}
...
for (var i =0; i<n; i++) {
    if (document.getElementById("sopa").getElementsByTagName("tr")[posiciones[i].fila].getElementsByTagName("td")[posiciones[i].columna].className !== "correcta"){
       if (document.getElementById("sopa").getElementsByTagName("tr")[posiciones[i].fila].getElementsByTagName("td")[posiciones[i].columna].style.background !== "rgb(65, 185, 65)") {
...
posiciones = generarsopa(palabras);
var x = new Array();
var y = new Array();
var tam = 0;
var inc = 0; 
...
$("#sopa tr td").click(function(){
   ...
   if (inc > 1){
       for (var i =0; i< tam; i++) 
        ...
        y = [];
        if (inc !== 4000)
            $("#mensajessopa").html("<span style='color:rgb(211, 33, 33);'><img src='images/cross.png' style='margin-bottom: -5px;'> Ups! por ah&iacute; no est&aacute; la palabra</span>");
        else
            $("#mensajessopa").html("<span style='color:rgb(0, 148, 252);'><img src='images/nota.png'> &iexcl;&Aacute;nimo! est&aacute;s cerca</span>");
...
$("#pista").click(function(){
    if (pistas_usadas < 3) {
     buscarPista();
         ...
         $("#mensajessopa").html("<span style='color:rgb(211, 33, 33);'><img src='images/cross.png' style='margin-bottom: -5px;'> Ups! ya no tienes m&aacute;s pistas</span>");
    }
...
$( "#fin" ).dialog({
  ...
  open: function(){
      play("hover"); 
      $("#retroalimentacion").html(retroalimentacion)
  },
  buttons: {
    "Cerrar": function(){ 
    $( this ).dialog( "close" );

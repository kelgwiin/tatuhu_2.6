...
//*****************Consulta de datos de instituto a modificar**********************
$query='SELECT * FROM tbl_unidadeducativa A WHERE codigo_colegio="'.$_GET['id'].'"';
$q0=mysql_query($query);
if(!$q0) die(mysql_error());
$_qa0=mysql_fetch_assoc($q0);
...
$estados = "<option></option>";
 while($qa0=mysql_fetch_assoc($qEst)){
	if($_qa0['id_estado']==$qa0['id_estado']) $aux='selected=selected';
     $estados.='<option '.$aux.' value="'.$qa0['id_estado'].'">'.$qa0['estado'].'</option>';
}
...
<table>
    <tr><td>
        ...
        <strong>'.$_qa0['codigo_colegio'].'</strong>
        </td>
        <td>
        Nombre del Instituto <span class="obligatorio">*</span><br>
        <input type="text" name="nombre_colegio" required class="input" id="nombre_colegio" value="'.$_qa0['nombre_colegio'].'">
        <br>
...
<table>
    ...
    <textarea id="direccion" name="direccion" class="input" rows="4" cols="50" value="'.$_qa0['direccion'].'"></textarea>
     </td>
    </tr>
</table>
<br>
...
<table>
    <tr>
        <td style="max-width:150px;">
            Tel&eacute;fono<br>
            <input type="text" name="telefono" id="telefono" class="input" style="width:100px;" value="'.$_qa0['telefono'].'">
            <br>
            <div id="error-telefono" class="msjError" style="max-width:150px;"></div>
        </td>
        <td>
            Correo<br>
            <input type="text" name="correo" id="correo" class="input" value="'.$_qa0['correo'].'">
            <br>
            <div id="error-correo" class="msjError"></div>
        </td>
        <td>
            Persona de Contacto<br>
            <input type="text" name="nombre_contacto" id="persona" class="input" value="'.$_qa0['nombre_contacto'].'">
           <br>
...
<a class="button glossy" href="instituto_visualizar.php"><span class="button-icon red-gradient"><span class="icon-tick"></span></span>Cancelar</a>
</div>
</form>
</div>
</div>
...
p_contenido('titulo_grande_1','Modificar Instituto Educativo');
p_contenido('migajas','<a href="index.php">Inicio</a>><a href="#">Modificar Instituto</a>');

p_con_pizarra(true);
p_contenido('pizarra','Actualizacion de un Instituto Educativo');

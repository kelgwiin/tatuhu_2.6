...
if (isset($_GET["id_area"]) && $_GET["id_area"] != "" &&
    isset($_GET["id_estudiante"]) && $_GET["id_estudiante"] != "" &&
    isset($_GET["nombre_est"]) && $_GET["nombre_est"] != "" &&
    isset($_GET["nombre_are"]) && $_GET["nombre_are"] != "" &&
    isset($_GET["id_grado"]) && $_GET["id_grado"] != ""){
echo '<h3 style="text-align:center;">Rendimiento del estudiante '.$_GET["nombre_est"].' en el &aacute;rea: '.$_GET["nombre_are"].'</h3><br>';
//Cantidad de actividades en total del area y de los grados del estudiante
...
$j = $j["total"];
if ($j > 0){
//Cantidad de actividades realizadas por el estudiante en un area de aprendizaje
$query ='SELECT E.tipo_actividad, COUNT(DISTINCT C.id_actividad) AS total
	 ...
	 WHERE A.id_persona=B.id_persona AND A.id_persona="'.$_GET["id_estudiante"].'"
...
if (mysql_numrows($q0) >0){
	...
	echo '<br><p style="text-align:center;">El estudiante ha realizado en total <span class="tip">'.$tEst.'</span> '.$text1.' del &aacute;rea.<br>
		     El &aacute;rea <i>'.$_GET["nombreAre"].'</i> tiene <span class="tip">'.$j.'</span> '.$text2.'.
                </p>
	    <h5>Cantidad de Actividades realizadas por el estudiante seg&uacute;n el tipo: </h5><br>';
	...
	echo '<div style="text-align:center; color:red;"><img src="../plantilla/img/standard/error.png"> El estudiante no han realizado actividades en esta &aacute;rea de aprendizaje</div>';
    }
}
else{

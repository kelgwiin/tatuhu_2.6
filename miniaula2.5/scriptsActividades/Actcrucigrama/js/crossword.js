...
var CrosswordUtils = {
    ...
    toHtml : function(grid, show_answers){
        ...
        var label = 1;
	//almacenamos el crucigrama en una estructura adicional
        for(var r = 0; r < grid.length; r++){
            html.push("<tr>");
            for(var c = 0; c < grid[r].length; c++){
                var cell = grid[r][c];
                ...
                if(is_start_of_word) {
                    ...
                    html.push("<td id="+label+" class='" + css_class + "letra init' title='" + r + ", " + c + "' style='background-image:url(" +img_url +");' >");
                    label++;			
                } else {
					if (char === "") //
						html.push("<td class='" + css_class + "' title='" + r + ", " + c + "'>");
					else
						html.push("<td class='" + css_class + "letra' title='" + r + ", " + c + "'>");
                }
